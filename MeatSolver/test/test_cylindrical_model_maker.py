...
from src.cylindrical_model_maker import CylindricalModelMaker
from src.pork_parameters import MIN_KAPPA, MAX_KAPPA


class TestCylindricalModelMaker(unittest.TestCase):
    _test_model_filepath = "model/test_model.tflite"

    def test_save_model(self):
        cylindrical_model = CylindricalModelMaker()
    ...
    def test_get_zero_contours(self):
        num_beta = 10
        cylindrical_model = CylindricalModelMaker(num_beta=num_beta)
    ...
    def test_compute_betas(self):
        num_beta = 3
        cylindrical_model = CylindricalModelMaker(num_beta=num_beta, nusselt_number=10)
    ...
    def _test_save(self, cylindrical_model: CylindricalModelMaker):
    ...
    def test_make_variables(self):
        num_rho = 10
        num_tau = 4
        cylindrical_model = CylindricalModelMaker(num_rho=num_rho, num_tau=num_tau)
